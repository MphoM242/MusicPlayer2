...
// Set initial volume and progress slider values
audio.volume = 0.5;  // Start volume at 50%
volumeSlider.value = 0.5;
progressSlider.value = 0;

// Show/hide volume slider on icon click
volumeIcon.addEventListener("click", toggleVolumeSlider);

// Close volume slider if clicked outside
...
document.getElementById("progress-slider").addEventListener("input", changeProgress);
document.getElementById("toggle-playlist").addEventListener("click", togglePlaylist);
document.querySelectorAll("#playlist li").forEach((song, index) => {
...
function loadSong(songIndex) {
  ...
  document.getElementById("current-song").textContent = `${songIndex + 1} / ${songs.length}`;
  document.getElementById("time").textContent = `0:00 / ${song.duration}`;
  audio.src = song.src;
  audio.currentTime=0;
  progressSlider.value = 0;
}
...
function togglePlayPause() {
  ...
  isPlaying = !isPlaying;
  document.getElementById("play").textContent = isPlaying ? "⏸️" : "▶️";
}
...
function changeProgress() {
  audio.currentTime = (audio.duration * document.getElementById("progress-slider").value) / 100;
}
